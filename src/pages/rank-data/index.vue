  ...
  <div class="p-4">
    ...
    <nut-cell v-if="retData.status">
      <div class=" w-full flex flex-col items-center">
        <div class="w-full text-base text-green-500 text-center my-2">查询成功</div>
        <nut-avatar class="py-2" size="64" :icon="retData.profileImg"></nut-avatar>
        <div class="flex w-full justify-between py-1">
          <div >大区</div><div>{{ retData.environment }}</div>
        </div>
        <div class="flex w-full justify-between py-1">
          <div >召唤师</div><div>{{ retData.summonerName }}</div>
        </div>
        <div class="flex w-full justify-between py-1">
          <div >单双排</div><div>{{ retData.soloRank }}</div>
        </div>
        <div class="flex w-full justify-between py-1">
          <div >灵活组排</div><div>{{ retData.flexRank }}</div>
        </div>
        <div class="flex w-full justify-between py-1">
          <div >当前赛季</div><div>{{ retData.currentSeason }}</div>
        </div>
        <div class="flex w-full justify-between py-1">
          <div >大区排名</div><div>{{ retData.allRank }}</div>
        </div>
      </div>
    </nut-cell>
  </div>
